 ...
 * - Promise.all([...])          : 실패가 없는 경우에만 성공 결과 모두 반환, 실패가 있으면 실패만 즉시 반환
 * - Promise.allSettled([...])   : 성공 실패 상관없이 모든 처리가 끝날 때까지 대기 후 모든 결과 반환
 * 
 * - Promise.race([...])         : 성공 실패 구분없이 가장 빠른 처리 결과 반환
 * - Promise.any([...])          : 가장 빨리 성공한 결과를 반환하거나 모두 실패한 경우 실패( AggregateError ) 반환
 * 
*/
